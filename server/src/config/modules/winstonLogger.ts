const chalk = require('chalk'); // 用于颜色化输出
import { createLogger, format, transports } from 'winston';
const DailyRotateFile = require('winston-daily-rotate-file');
const moment = require('moment');

// 定义日志级别颜色
const levelsColors: any = {
  error: 'red',
  warn: 'yellow',
  info: 'green',
  debug: 'blue',
    ...
    new transports.Console({
      format: format.combine(
        ...
        format.printf((info) => {
          ...
          const message = `${chalk.blue(moment(info.timestamp).format('YYYY-MM-DD HH:mm:ss'))} ${chalkColor(info[symbols[2]])}`;
          return message;
        }),
      ),
      level: 'debug',
